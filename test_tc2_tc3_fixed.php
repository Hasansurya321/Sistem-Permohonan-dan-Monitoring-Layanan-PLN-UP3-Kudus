...
// TC3: Simulasi SUKSES - attempts STAYS 1, payment=SUCCESS, status->VERIFIKASI_DATA
$payment->status = 'SUCCESS';  // FIXED: bukan PAID tapi SUCCESS
$payment->paid_at = now();
$payment->save();
...
$tc3Pass = ($sr->payment_attempt_count == 1 && $payment->status == 'SUCCESS' && $sr->status_detail == PermohonanDetailStatus::VERIFIKASI_DATA);
echo ($tc3Pass ? "✅ TC3 PASS" : "❌ TC3 FAIL") . PHP_EOL;

echo PHP_EOL . "=== RINGKASAN ===" . PHP_EOL;
echo "TC2 (Gagal 1x): " . ($tc2Pass ? "✅ PASS" : "❌ FAIL") . PHP_EOL;
echo "TC3 (Sukses):   " . ($tc3Pass ? "✅ PASS" : "❌ FAIL") . PHP_EOL;
