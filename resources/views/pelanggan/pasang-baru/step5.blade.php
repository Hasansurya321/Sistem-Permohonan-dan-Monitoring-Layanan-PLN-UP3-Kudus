...
<div class="max-w-4xl mx-auto py-10">
    ...
    <div class="bg-white/70 backdrop-blur-lg rounded-3xl border border-white/50 shadow-xl overflow-hidden relative">
        <div class="p-8 md:p-12 relative z-10 w-full">
            <h2 class="text-2xl font-bold text-slate-800 mb-2">Lengkapi Data Pemohon</h2>
            <p class="text-slate-500 mb-8">Mohon lengkapi data administratif berikut untuk menyelesaikan permohonan pasang baru.</p>

            <form id="formStep5" action="{{ route('pasang-baru.step5.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="bg-blue-50/50 rounded-xl p-6 border border-blue-100 mb-8">
                    ...
                    <!-- Info Box: ID Pelanggan will be assigned later -->
                    <div class="mt-4 p-4 bg-amber-50 rounded-lg border border-amber-200">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="text-sm text-amber-800 font-semibold">ID Pelanggan Akan Diberikan Kemudian</p>
                                <p class="text-xs text-amber-600 mt-1">Karena ini adalah permohonan pasang baru, ID Pelanggan akan diberikan oleh PLN setelah permohonan Anda disetujui.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-between mt-10">
                    <a href="{{ route('pasang-baru.step4') }}" class="px-8 py-3 rounded-xl font-bold bg-slate-200 text-slate-700 hover:bg-slate-300 transition">
                    ...
                    <button type="submit" id="btn-submit" disabled 
                        ...
                        <span id="submit-text">Ajukan Pasang Baru</span> <i class="fas fa-arrow-right ml-2"></i>
                    </button>
                </div>
            </form>
        </div>
...
async function verifyKK() {
    ...
    try {
        const res = await fetch("{{ route('pasang-baru.verify-kk') }}", {
            method: 'POST',
            headers: {
...
async function verifyNPWP() {
    ...
    try {
        const res = await fetch("{{ route('pasang-baru.verify-npwp') }}", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': "{{ csrf_token() }}"
